...
class LoadImages:  # for inference
    ...
    def xunhuan(self):#定义提取帧的循环
        ...
        self.yuzhi=13
        self.timeF = 15#每隔多少帧读取三个
        crop_rate=0.96#视频裁剪比
    ...
    def __next__(self):#含有__next__()函数的对象都是一个迭代器
        ...
        if self.video_flag[self.count]:
            ...
            thr=self.xunhuan()
            while thr>=0:  
                if thr<=self.yuzhi or self.frame >= (self.nframes-40) :
                    img0=self.lastframe[(self.timeF//2)]
                    self.keyframe+=1
                    break
                if thr>self.yuzhi and self.frame < (self.nframes-40):
                    thr=self.xunhuan()
