...
class LoadStreams:  # multiple IP or RTSP cameras多个IP或RTSP摄像机
    def __init__(self, sources='streams.txt', img_size=416):
        ...
        for i, s in enumerate(sources):#s应该是路径
            # Start the thread to read frames from the video stream从视频流读取帧
            ...
            _, self.imgs[i] = cap.read()  # guarantee first frame保证第一帧
            
            #多线程，_thread.start_new_thread ( function, args[, kwargs] )
            #function - 线程函数。args - 传递给线程函数的参数,他必须是个tuple类型。
            #kwargs - 可选参数
    ...
    #    # Read next stream frame in a daemon thread多线程读取下一个流帧
    #    n = 0
    #    while cap.isOpened():
    #        n += 1
    #        # _, self.imgs[index] = cap.read()
    #        cap.grab()#从视频文件或相机中抓取下一帧。true为抓取成功
    #        if n == 4:  # read every 4th frame
    #            _, self.imgs[index] = cap.retrieve()#解码并返回一个帧
    #            n = 0
